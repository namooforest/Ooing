...
function scrapePageTables() {
  ...
  if (pageUrl.includes('samsungcard.com') || htmlSnippet.includes('samsungcard') || htmlSnippet.includes('삼성카드')) {
    ...
    // "총 N건" — 해외이용 섹션을 제외한 텍스트에서 합산
    const countText = removeForeignSection(result.pageText);
...
function parseSamsungText(text) {
  ...
  // "해외이용 상세정보" 섹션만 제외 (이후 "연회비/기타수수료" 등은 포함)
  const targetText = removeForeignSection(text);
...
/**
 * "해외이용 상세정보" 섹션만 제거하고 이후 텍스트(연회비/기타수수료 등)는 유지
 */
function removeForeignSection(text) {
  const foreignIdx = text.search(/해외\s*이용\s*(상세|내역)/);
  if (foreignIdx <= 0) return text;

  const before = text.substring(0, foreignIdx);
  const after = text.substring(foreignIdx);

  const nextSection = after.search(
    /\n[^\n]*(연회비|기타\s*수수료|단기\s*카드\s*대출|리볼빙|포인트\s*안내|이용\s*한도|SMS\s*이용)/
  );
  if (nextSection > 0) {
    return before + after.substring(nextSection);
  }
  return before;
}
